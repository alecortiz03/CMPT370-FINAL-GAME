...
class SegHandler:
    ...
    def draw_solid_wall_range(self, x1, x2):
        ...
        world_front_z1 = front_sector.ceiling_height - self.player.height
        ...
        hypotensue = math.dist(self.player.pos, seg.start_vertex)
        rw_distance = hypotensue * math.cos(math.radians(offset_angle))
        ...
        for x in range(x1, x2 + 1):
            ...
            if b_draw_ceil:
                ...
                render.draw_vline(x, cy1, cy2, ceil_texture, light_level)
            
            if b_draw_wall:
                wy1 = int(max(draw_wall_y1, upper_clip[x] + 1))
                wy2 = int(min(draw_wall_y2, lower_clip[x] - 1))
                render.draw_vline(x, wy1, wy2, wall_texture, light_level)

            if b_draw_floor:
                fy1 = int(max(draw_wall_y2 + 1, upper_clip[x] + 1))
                fy2 = lower_clip[x] - 1
                render.draw_vline(x, fy1, fy2, floor_texture, light_level)
    ...
    def draw_portal_wall_range(self, x1, x2):
        ...
        world_front_z1 = front_sector.ceiling_height - self.player.height
        world_back_z1 = back_sector.ceiling_height - self.player.height
        world_front_z2 = front_sector.floor_height - self.player.height
        world_back_z2 = back_sector.floor_height - self.player.height

        if (world_back_z1 != world_front_z1 or
    ...
    def init_floor_ceil_height(self):
    ...
    def classify_segment(self, segment, x1, x2, rw_angle1):
        ...
        if (front_sector.ceiling_height != back_sector.ceiling_height or
            front_sector.floor_height != back_sector.floor_height):
            self.clip_portal_walls(x1, x2)
            return None
        
        if (back_sector.ceiling_texture == front_sector.ceiling_texture and
